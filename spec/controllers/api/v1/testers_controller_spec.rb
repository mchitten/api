  ...
  describe 'GET #errors' do
    ...
    describe 'with an exception handler that raises the error' do
      it 'raises the error' do
        ...
        allow(QuirkyApi).to receive_message_chain(:config, :exception_handler).and_return(->(e) { raise e })
      ...
      context 'if validate_associations is true' do
        context 'if you specify an invalid association' do
          it 'returns an error' do
            allow(QuirkyApi).to receive_message_chain(:config, :validate_associations).and_return(true)
      ...
      context 'if validate_associations is false' do
        context 'if you specify an invalid association' do
          it 'does not throw an error' do
            allow(QuirkyApi).to receive_message_chain(:config, :validate_associations).and_return(nil)
            get :index, format: 'json', associations: ['octopus']
            expect(response.body).to eq([
              {
                id: 1,
